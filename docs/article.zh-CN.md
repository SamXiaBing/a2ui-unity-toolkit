# 车机端生成式 UI 实践：A2UI 协议的 Unity UI Toolkit 原生渲染方案

> 原载于（2026-09）：[微信公众号](https://mp.weixin.qq.com/s/UFqDQDhYucKRuFd-JyHMOg) ｜ [知乎](https://zhuanlan.zhihu.com/p/2079675042464523426)。本页为仓库归档版，图片改用仓库内资源。英文版见 [article.en.md](article.en.md)。
...
![A2UI 渲染流水线七阶段](article-assets/1.png)

### 数据驱动模式

用户操作不直接修改界面，而是经动作路由进入车机服务，服务回写数据后界面随数据自动更新：

![数据驱动模式](article-assets/2.png)
...
![Figma 转换管线](article-assets/4.png)
...
> 具体信息写在 [docs/engine-compat-tuanjie.md](engine-compat-tuanjie.md)

## 回归测试体系

这套生成式 UI 的典型风险是改动之后，可能导致多处界面异常，人工验证成本高，所以写了自动化回归测试，以期能减少一些问题。

![回归测试体系](article-assets/5.png)
...
> clone 仓库、用 Tuanjie 打开（Unity 2022.3 兼容 fork，标准 Unity 未验证）、在 Play 之后，打开 A2UISchemeA 的测试发送面板，选择样例并发送。也可以模拟 Agent 推流去执行一条 Python 命令
>
> - 仓库地址：[https://github.com/SamXiaBing/a2ui-unity-toolkit](https://github.com/SamXiaBing/a2ui-unity-toolkit)
>
> 动手改代码前，建议先看一眼 [docs/engine-compat-tuanjie.md](engine-compat-tuanjie.md)，这里有之前踩坑的清单。
>
